...
func _track_combo_phases() -> void:
	...
	if current_combo_phase != _last_combo_phase:
		if current_combo_phase != -1:
			var key = String(&"attack.fast") + str(current_combo_phase)
			_activation_counts[key] = _activation_counts.get(key, 0) + 1
...
func _update_labels() -> void:
	...
	for tag in tags:
		...
		if tag in TAG_COLORS:
			display_state = s_tag.replace("motion.", "").replace("status.", "").capitalize()
		...
		if s_tag.begins_with("motion."):
			...
			context_list.append(s_tag)
	
	var left_text = "State: %s\n" % display_state
	left_text += "Context: %s\n" % (", ".join(context_list) if not context_list.is_empty() else "")
	left_text += "Motion: %s\n" % motion_val
	left_text += "Jump: %s\n" % jump_val
	left_text += "Attack: %s\n" % attack_val
	left_text += "Physics: %s\n" % physics_val
